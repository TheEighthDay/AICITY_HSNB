...
from .postprocessing import postprocess_results, postprocess_results_tkb
...
def valid_one_epoch(
    ...
    if evaluator is not None:
        if (ext_score_file is not None) and isinstance(ext_score_file, str):
            results = postprocess_results_tkb(results, ext_score_file)
        # call the evaluator
        f=open(output_file,"w")
        for (video,ts,te,label,score) in zip(results['video-id'],results['t-start'],results['t-end'],results['label'],results['score']):
            f.write("{} {} {} {} {}\n".format(video,ts,te,label,score))
        f.close()
...
def val_one_epoch_lkc(
    ...
    if model_ema:
        prefix = 'ema_val'
        print("\n[EMA_Val]: Epoch {:d} started".format(curr_epoch))
    else:
        prefix = 'val'
        print("\n[Val]: Epoch {:d} started".format(curr_epoch))
    start = time.time()
    for iter_idx, video_list in enumerate(val_loader, 0):
        # forward the model
        losses = model(video_list, val=True)
    ...
    flag = False
    if losses_tracker['reg_loss'].avg < min_val:
        min_val = losses_tracker['reg_loss'].avg
        flag = True

    # finish up and print
    if model_ema:
        print("[EMA_Val]: Epoch {:d} finished.\n".format(curr_epoch))
        print("Mini ema_reg_loss {:.4f}.\n".format(min_val))
    else:
        print("[Val]: Epoch {:d} finished.\n".format(curr_epoch))
        print("Mini reg_loss {:.4f}.\n".format(min_val))
    return min_val, flag