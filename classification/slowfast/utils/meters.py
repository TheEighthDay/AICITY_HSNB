...
class ExtractMeter(object):
    ...
    def __init__(
        ...
        self.ensemble_method = ensemble_method
        # Initialize tensors.
        self.video_feas = torch.zeros((num_videos, 2048))
        self.video_preds = torch.zeros((num_videos, num_cls))

        self.video_start = torch.zeros((num_videos, 1))
        self.video_end = torch.zeros((num_videos, 1))
    ...
    def reset(self):
        ...
        """
        self.clip_count.zero_()
        self.video_feas.zero_()
        self.video_preds.zero_()
        self.video_start.zero_()
        self.video_end.zero_()

    def update_stats(self, feas, preds, clip_ids, start, end):
        """
        Collect the predictions from the current batch and perform on-the-flight
        summation as ensemble.
        """
        for ind in range(preds.shape[0]):
            vid_id = int(clip_ids[ind]) // self.num_clips
            self.video_start[vid_id] = start[ind]
            self.video_end[vid_id] = end[ind]
            if self.ensemble_method == "sum":
                self.video_feas[vid_id] += feas[ind]
                self.video_preds[vid_id] += preds[ind]
            else:
                raise NotImplementedError(
                    "Ensemble Method {} is not supported".format(
...
class ProposalMeter(object):
    ...
    def __init__(
        ...
        self.video_frame = torch.zeros((num_videos, 1))
        self.video_start = torch.zeros((num_videos, 1))
    ...
    def reset(self):
        ...
        self.clip_count.zero_()
        self.video_preds.zero_()
        self.video_frame.zero_()
        self.video_start.zero_()

    def update_stats(self, preds, clip_ids, frame, start):
        """
        Collect the predictions from the current batch and perform on-the-flight
        summation as ensemble.
        """
        for ind in range(preds.shape[0]):
            vid_id = int(clip_ids[ind]) // self.num_clips
            self.video_frame[vid_id] = frame[ind]
            self.video_start[vid_id] = start[ind]
            if self.ensemble_method == "sum":
                self.video_preds[vid_id] += preds[ind]
            else:
...
class BinaryTestMeter(object):
    ...
    def __init__(
        ...
        self.max_binary1_recal = 0.
        self.max_binary1_precision = 0.
        self.max_binary1_f1 = 0.
        self.clip_count = torch.zeros((num_videos)).long()
        self.topk_accs = []
        self.stats = {}
    ...
    def finalize_metrics(self, ks=(1, 5)):
        ...
        if self.multi_label:
            ...
            pre = self.video_preds.softmax(-1).argmax(-1)
            truth = self.video_labels
            binary1_recal = ((pre[torch.where(truth == 1)] == 1).sum() / len(torch.where(truth == 1)[0])).item()
            binary1_precision = ((pre[torch.where(truth == 1)] == 1).sum() / len(torch.where(pre == 1)[0])).item()
            binary1_f1 = 2 * binary1_recal * binary1_precision / (binary1_recal + binary1_precision)
            self.max_binary1_acc = max(self.max_binary1_recal, binary1_recal)
            self.max_binary1_precision = max(self.max_binary1_precision, binary1_precision)
            self.max_binary1_f1 = max(self.max_binary1_f1, binary1_f1)

            logger.info(f'Binary 1 recal: {binary1_recal * 100:.3f}')
            logger.info(f'Binary 1 precision: {binary1_precision * 100:.3f}')
            logger.info(f'Binary 1 f1: {binary1_f1 * 100:.3f}')
            logger.info(f'[MAX] Binary 1 recal: {self.max_binary1_acc * 100:.3f}')
            logger.info(f'[MAX] Binary 1 precision: {self.max_binary1_precision * 100:.3f}')
            logger.info(f'[MAX] Binary 1 f1: {self.max_binary1_f1 * 100:.3f}')

            if binary1_f1 == self.max_binary1_f1:
                flag = True
