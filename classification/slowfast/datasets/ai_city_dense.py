...
class Ai_city_dense(torch.utils.data.Dataset):
    """
    ...
    def get_seq_frames(self, index, temporal_sample_index):
        ...
        frame_index = torch.linspace(start_idx, end_idx, num_frames)
        seq = torch.clamp(frame_index, 0, video_length - 1).long()
        ...
        if self.client:
            frames = torch.as_tensor(
                utils.load_images_from_ceph(
                    [path_template.format(frame.item()) for frame in seq],
                    self.client,
            ))
        else:
            frames = torch.as_tensor(
