...
from slowfast.datasets.sampler import DistributedSamplerWrapper,BatchBalanceClassSampler,BalanceClassSampler
...
def construct_loader(cfg, split, is_precise_bn=False):
    ...
    if isinstance(dataset, torch.utils.data.IterableDataset):
        ...
        print("Not torch.utils.data.IterableDataset")
        if (
            ...
            print("Start build sampler")
            if cfg.DATA.BALANCE and (split in ["train"]):
                
                # Create a sampler for multi-process training
                train_labels = dataset._labels
                # print(train_labels)
                # sampler = utils.create_sampler(dataset, shuffle, cfg)
                sampler = BalanceClassSampler(train_labels, mode=cfg.DATA.BALANCE_mode )#  mode downsampling, upsampling  ,BatchBalanceClassSampler(train_labels, num_classes=cfg.MODEL.NUM_CLASSES, num_samples=(cfg.TRAIN.BATCH_SIZE//cfg.MODEL.NUM_CLASSES+1))
