...
def decode(
    ...
    try:
        ...
        elif backend == "decord" or backend == "decord_GIC" or backend == "decord_AdaptGIC":
            frames = container
        elif backend == "frame":
            pass
        else:
    ...
    if backend in ["pyav", "torchvision"]:
    ...
    elif backend == "decord_GIC" or backend == "decord_AdaptGIC":
        clip_sz = sampling_rate * num_frames
        start_idx, end_idx = get_start_end_idx(
            len(frames),
            clip_sz,
        ...
        # print("---------index",index)
        tmp_frames = [frames[i.item()] for i in index]
        print("tmp_framessssss_pre[0]",tmp_frames[0].size(),tmp_frames[0])
        for i in range(len(tmp_frames)):
            if backend == "decord_AdaptGIC":
                ...
                if torch.mean(k) > 0:
                    ...
                    print("-----torch.mean(k):",torch.mean(k))
                    gamma_val = 0.45
            else:
                gamma_val = 0.45
            ...
            tmp_frames[i] = GIC
        print("tmp_framessssss_after[0]",tmp_frames[0].size(),tmp_frames[0])
        ...
        index = torch.linspace(start_idx, end_idx, num_frames)
        index = torch.clamp(index, 0, len(glob_frame_path) - 1).long()
