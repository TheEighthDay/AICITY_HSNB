...
model_path = {
    's_vip_nods_conv_s7': '/mnt/lustre/share_data/zongzhuofan/final/lkc/left_model/vip/s_vip_nods_conv_s7.pth',
    's_vip_nods_conv_m7': '/mnt/lustre/share_data/zongzhuofan/final/lkc/left_model/vip/s_vip_nods_conv_m7.pth',
}
...
class WeightedPermuteMLP2(nn.Module):
...
class PermutatorBlock(nn.Module):
    def __init__(self, dim, segment_dim, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.t_attn = TemporalAttention(dim, segment_dim=segment_dim, qkv_bias=qkv_bias, qk_scale=None,
                                        attn_drop=attn_drop)
    ...
    def forward(self, x):
        xt = x + self.t_attn(self.t_norm1(x))
        x = x + self.drop_path(self.attn(self.norm1(xt))) / self.skip_lam
...
class SequentialVisionPermutator_16(nn.Module):
    """ Vision Permutator
    """
    ...
    def __init__(self, cfg):
        ...
        layers = cfg.VIP.LAYERS
        transitions = cfg.VIP.TRANSITIONS
        segment_dim = cfg.VIP.SEGMENT_DIM
        t_stride = cfg.VIP.T_STRIDE
        mlp_ratios = cfg.VIP.MLP_RATIOS
        embed_dims = cfg.VIP.EMBED_DIMS
        patch_size = cfg.VIP.PATCH_SIZE
        qkv_bias = cfg.VIP.QKV_BIAS
        qk_scale = cfg.VIP.QKV_SCALE
        attn_drop_rate = cfg.VIP.ATTENTION_DROPOUT_RATE
        drop_path_rate = cfg.VIP.DROP_DEPTH_RATE
        norm_layer = nn.LayerNorm
        mlp_fn = [WeightedPermuteMLP] * 3 + [WeightedPermuteMLP2]
        skip_lam = 1.0
        ...
        for name, p in self.named_parameters():
            ...
            if 't_attn.mlp_t.weight' in name:
                nn.init.constant_(p, 0)
            if 't_attn.mlp_t.bias' in name:
                nn.init.constant_(p, 0)
    ...
    def get_pretrained_model(self, cfg):
        if cfg.VIP.PRETRAIN_NAME:
            checkpoint = torch.load(model_path[cfg.VIP.PRETRAIN_NAME], map_location='cpu')
