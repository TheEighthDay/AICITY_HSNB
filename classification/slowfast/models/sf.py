...
model_path = '/mnt/lustre/share_data/tiankaibin/SF'
model_path = {
...
class SF(nn.Module):
    ...
    def forward(self,inputs,snipt=False):
        if snipt:
...
class SF2(nn.Module):
    def __init__(self, cfg):
        ...
        self.dropout = nn.Dropout(cfg.SF.DROPOUT_RATE)
        self.projection = nn.Linear(2048, cfg.MODEL.NUM_CLASSES, bias=True)
        self.projection2 = nn.Linear(2048, 2, bias=True)
    ...
    def forward_2head(self,inputs):
        out = self.forward_features(inputs)
        out = out.flatten(2).mean(-1)
        out = self.dropout(out)
        out2 = self.projection2(out)
        out = self.projection(out)
        return out,out2
