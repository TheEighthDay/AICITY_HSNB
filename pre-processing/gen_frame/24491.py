...
def save_clip(flag,base_path,start_frame,frame_rate,second,vr,video_name,seg,class_id,last_class_id,next_class_id,User_ID,Camera_View,Appearance_Block):
    ...
    for i, seg_ind in enumerate(range(start_frame, end_frame)):
        ...
        # print(name)
csv_list = []
with open("/mnt/cache/shangjingjie1/user_id_24491.csv")as f:
    f_csv = csv.reader(f)
...
for i, row in enumerate(csv_list):
    if i!=0:
        if row[1]!=''and row[1]!='File Name':
            ...
            if video_name == "Rightside_window_user_id_24491_NoAudio_1.MP4":
                video_name = "Rightside_window_user_id_24491_NoAudio_1.mp4"
            ...
            #     start_min = 0
            #     start_sed = 0
            #     end_min = int(row[4].split(":")[0])
            #     end_sed = int(row[4].split(":")[1])
        ...
        # print(row)
        # print("------normal-------")
        start_min = int(row[4].split(":")[0])
        start_sed = int(row[4].split(":")[1])
        end_min = int(row[5].split(":")[0])
        end_sed = int(row[5].split(":")[1])+1
        if start_min != end_min:
            temp = 60 * (end_min - start_min)
        ...
        if i != len(csv_list) and csv_list[i + 1][4] != row[5] and csv_list[i+1][1]=='':
            # print("------margin-------")
            temp = 0
            start_min = int(row[5].split(":")[0])
            start_sed = int(row[5].split(":")[1])+1
            end_min = int(csv_list[i + 1][4].split(":")[0])
            end_sed = int(csv_list[i + 1][4].split(":")[1])
            # print("start %d:%d"%(start_min,start_sed))
            # print("end %d:%d" % (end_min, end_sed))
            if start_min != end_min:
                temp = 60 * (end_min - start_min)
