...
# 5. 关闭文件
# f.close()
def save_clip(flag,base_path,start_frame,frame_rate,second,vr,video_name,seg,class_id,last_class_id,next_class_id,User_ID,Camera_View,Appearance_Block,total_frame):
...
for i, row in enumerate(csv_list):
    if i!=0:
        if row[1]!=''and row[1]!='File Name':
            ...
            video_name = (row[1][:-2]+"_NoAudio"+row[1][-2:]+".MP4")
            if video_name == "Rightside_window_User_id_38058_NoAudio_0.MP4":
                video_name = "Right_side_window_User_id_38058_NoAudio_0.MP4"
            if video_name == "Rightside_window_User_id_38058_NoAudio_1.MP4":
                video_name = "Right_side_window_User_id_38058_NoAudio_1.MP4"
        ...
        # else:
        #     next_class_id = csv_list[i + 1][6]
        save_clip(flag,base_path, start_frame, frame_rate, second, vr, video_name,cnt,class_id,last_class_id,next_class_id,User_ID,Camera_View,Appearance_Block,total_frame)
        # print("------normal-------")
        if i != len(csv_list) and csv_list[i + 1][4] != row[5] and csv_list[i+1][1]=='':
            # print("------margin-------")
            temp = 0
